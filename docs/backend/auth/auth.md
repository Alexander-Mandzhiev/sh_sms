...
#### 7.1 Получение сессий пользователя (ListSessionsForUser)

*Назначение:* 

Получение списка сессий для конкретного пользователя с возможностью фильтрации по клиенту и приложению.
...
message SessionFilter {
    ...
    bool active_only = 4;      // Только активные сессии (default: false)
    int32 page = 5;            // Номер страницы (начиная с 1)
    int32 count = 6;           // Количество на странице (1-1000)
}
```
*Пример запроса:*
...
```json
{
    "sessions": [
        {
        "session_id": "2a54a230-fa3a-4b7a-8494-dec9b99fade3",
        "created_at": {"seconds": 1747726942},
        "last_activity": {"seconds": 1747726942},
        "client_id": "8268ec76-d6c2-48b5-a0e4-a9c2538b8f48",
...
```
*Особенности:*
- Доступ только для владельца аккаунта или администратора
- Возвращает расширенные данные пользователя (имя, email, телефон)

*Фильтрация по:*
- Активным сессиям (active_only=true)
- Конкретному приложению (app_id)
- Клиенту (client_id)
- Пагинация с лимитом 100 записей на страницу
...
```
#### 7.2 Получение всех сессий (ListAllSessions)
*Назначение:*

Административный метод для получения всех сессий в системе с расширенной фильтрацией.
...
message AllSessionsFilter {
    ...
    int32 app_id = 2;              // ID приложения (опционально)
    int32 page = 3;                // Номер страницы (начиная с 1)
    int32 count = 4;               // Количество на странице (1-100)
    optional bool active_only = 5; // Только активные сессии
    optional string full_name = 6; // Фильтр по имени пользователя
    optional string phone = 7;     // Фильтр по телефону
    optional string email = 8;     // Фильтр по email
}
```
*Пример запроса:*
...
```json
{
  "sessions": [
    {
      "session_id": "b569705e-e3e9-4a1a-9402-c4710060f966",
      "created_at": {
        "seconds": "1747736367",
        "nanos": 619838000
      },
...
- По данным пользователя (email, телефон, имя)
- По активности сессии
- По приложению и клиенту
- Оптимизирован для больших наборов данных
- Возвращает полную информацию о пользователе

*Безопасность:*
- Логирование всех запросов
- Ограничение 10 запросов/минуту на администратора
- Шифрование персональных данных в ответе
...
*Общие особенности для методов сессий:*
...
- Автоматический расчет total_count
- Сортировка по времени последней активности


*Безопасность:*
- Маскирование последнего октета IP-адресов
- Санкционирование User-Agent
- Валидация UUID форматов
#### 7.3 Завершение сессии (TerminateSession)
gRPC Contract
```protobuf
