...
func (h *Handler) getRole(c *gin.Context) {
	...
	if err != nil {
		logger.Error("Failed to get role", slog.String("role_id", id), slog.String("client_id", clientID), slog.Int("app_id", appID), slog.String("error", err.Error()))
	...
	logger.Info("Role retrieved successfully", slog.String("role_id", role.ID), slog.String("client_id", role.ClientID), slog.Int("app_id", int(role.AppID)))
