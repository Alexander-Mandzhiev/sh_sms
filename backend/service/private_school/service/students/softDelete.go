...
func (s *Service) SoftDeleteStudent(ctx context.Context, id, clientID uuid.UUID) error {
	const op = "service.PrivateSchool.Students.SoftDeleteStudent"
	...
	if err := s.provider.SoftDeleteStudent(ctx, id, clientID); err != nil {
		return s.handleRepoError(err, op)
	}

	logger.Info("student soft deleted successfully", "student_id", id.String(), "client_id", clientID.String())
	return nil
}
