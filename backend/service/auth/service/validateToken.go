...
func (s *AuthService) ValidateToken(ctx context.Context, token string, tokenTypeHint string) (*models.TokenValidationResult, error) {
	...
	if err != nil {
		if errors.Is(err, handle.ErrSessionNotFound) {
			...
			return &models.TokenValidationResult{Active: false}, nil
		}
	...
	return &models.TokenValidationResult{
		Active:    time.Now().Before(claims.ExpiresAt.Time),
