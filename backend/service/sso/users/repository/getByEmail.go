...
func (r *Repository) GetByEmail(ctx context.Context, clientID uuid.UUID, email string) (*models.User, error) {
	...
	query := `SELECT id, client_id, email, password_hash, full_name, phone, is_active, created_at, updated_at, deleted_at FROM users WHERE email = $1 AND client_id = $2`

	var user models.User
	err := r.db.QueryRow(ctx, query, email, clientID).
