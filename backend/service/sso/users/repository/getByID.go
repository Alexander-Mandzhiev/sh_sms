...
func (r *Repository) GetByID(ctx context.Context, clientID, userID uuid.UUID) (*models.User, error) {
	...
	query := `SELECT id, client_id, email, password_hash, full_name, phone, is_active, created_at, updated_at, deleted_at FROM users WHERE id = $1 AND client_id = $2`

	var user models.User
	err := r.db.QueryRow(ctx, query, userID, clientID).
