...
func (r *Repository) Update(ctx context.Context, role *models.Role) (*models.Role, error) {
	...
	query := `UPDATE roles SET name = $1, description = $2, level = $3, updated_at = NOW() 
		WHERE id = $4 AND client_id = $5 AND app_id = $6 AND deleted_at IS NULL 
	...
	err := r.db.QueryRow(ctx, query, role.Name, role.Description, role.Level, role.ID, role.ClientID, role.AppID).
		Scan(&updated.ID, &updated.ClientID, &updated.Name, &updated.Description, &updated.Level, &updated.IsCustom,
			&updated.IsActive, &updated.CreatedBy, &updated.CreatedAt, &updated.UpdatedAt, &updated.DeletedAt)
	if err != nil {
